...
const ConstructionSafetyDetector = ({ onBack }) => {
    ...
    const detectFrame = async () => {
        ...
        canvas.toBlob(async (blob) => {
            ...
            try {
                const response = await fetch(`${API_URL}/api/detect-construction-safety`, {
                    method: 'POST',
                    body: formData
                });
    ...
    return (
        <div className="mt-6 flex flex-col items-center w-full">
            <h2 className="text-xl font-semibold mb-4 text-center">Construction Safety Detector</h2>
            ...
            <button
                ...
                className={`w-full max-w-md py-3 px-4 rounded-lg text-white font-medium shadow-md transition transform active:scale-95 ${isDetecting ? 'bg-orange-600 hover:bg-orange-700' : 'bg-gray-600 hover:bg-gray-700'}`}
            >
                {isDetecting ? 'Stop Detection' : 'Start Live Detection'}
            </button>

            <p className="text-sm text-gray-500 mt-2 text-center max-w-md">
                Monitor construction sites for safety compliance (helmets, hazardous zones).
            </p>
