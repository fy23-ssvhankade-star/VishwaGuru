  ...
  return (
    <div className="p-4 max-w-md mx-auto">
          ...
          <div className="mb-4 rounded-lg overflow-hidden shadow-lg border-2 border-gray-300 bg-gray-100 min-h-[300px] relative">
            {!imgSrc ? (
              ...
              <div className="relative">
                  ...
                  {detections.length > 0 && (
                      <div className="absolute top-0 left-0 right-0 bg-orange-600 text-white p-2 text-center font-bold opacity-90">
      ...
      <div className="flex justify-center gap-4">
        {!imgSrc ? (
          <button onClick={capture} disabled={!!cameraError} className="bg-orange-600 text-white px-6 py-2 rounded-full font-semibold shadow-md hover:bg-orange-700 transition">Capture Photo</button>
        ) : (
          <>
            ...
            <button onClick={detectVehicle} disabled={loading} className="bg-orange-600 text-white px-6 py-2 rounded-full font-semibold shadow-md hover:bg-orange-700 transition flex items-center">{loading ? 'Analyzing...' : 'Detect Vehicle'}</button>
          </>
        )}
      </div>
      <p className="mt-4 text-sm text-gray-600 text-center">Point camera at a vehicle to check for abandonment signs.</p>
    </div>
  );
};
