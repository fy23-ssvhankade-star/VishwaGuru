...
const PublicFacilitiesDetector = ({ onBack }) => {
  ...
  const detectFacilities = async () => {
    ...
    try {
        ...
        // Call Backend API
        const response = await fetch('/api/detect-public-facilities', {
            method: 'POST',
            body: formData,
        });
        ...
        if (response.ok) {
            ...
            if (data.detections.length === 0) {
                alert("No facility issues detected.");
            }
        } else {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 max-w-md mx-auto w-full">
        ...
        <div className="flex justify-center gap-4">
          {!imgSrc ? (
            ...
            <>
              ...
              <button
                onClick={detectFacilities}
                disabled={loading}
                className={`bg-blue-600 text-white px-6 py-2 rounded-full font-semibold shadow-md hover:bg-blue-700 transition flex items-center ${loading ? 'opacity-70 cursor-wait' : ''}`}
              >
                {loading ? (
                    <>
        ...
        <p className="mt-4 text-sm text-gray-600 text-center">
          Point camera at benches, fences, or playgrounds to check for damage.
        </p>
      </div>
    </div>
  );
