...
const PublicFacilitiesDetector = ({ onBack }) => {
    ...
    const detectFrame = async () => {
        ...
        canvas.toBlob(async (blob) => {
            ...
            try {
                const response = await fetch(`${API_URL}/api/detect-public-facilities`, {
                    method: 'POST',
                    body: formData
                });
    ...
    return (
        <div className="mt-6 flex flex-col items-center w-full">
            <h2 className="text-xl font-semibold mb-4 text-center">Public Facilities Detector</h2>
            ...
            <button
                ...
                className={`w-full max-w-md py-3 px-4 rounded-lg text-white font-medium shadow-md transition transform active:scale-95 ${isDetecting ? 'bg-blue-600 hover:bg-blue-700' : 'bg-green-600 hover:bg-green-700'}`}
            >
                {isDetecting ? 'Stop Detection' : 'Start Live Detection'}
            </button>

            <p className="text-sm text-gray-500 mt-2 text-center max-w-md">
                Detect issues with public benches, parks, or facilities.
            </p>
