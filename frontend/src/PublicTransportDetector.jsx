...
import { Camera, RefreshCw, AlertTriangle, Bus } from 'lucide-react';
...
const PublicTransportDetector = ({ onBack }) => {
    ...
    const analyze = async () => {
        ...
        canvas.toBlob(async (blob) => {
            ...
            try {
                const data = await detectorsApi.publicTransport(formData);
    ...
    return (
        <div className="flex flex-col items-center w-full max-w-md mx-auto p-4">
            <div className="w-full flex justify-between items-center mb-4">
                <button onClick={onBack} className="text-blue-600 font-bold">&larr; Back</button>
                ...
                {analyzing && (
                    <div className="absolute inset-0 bg-black/50 flex items-center justify-center backdrop-blur-sm">
                        <div className="flex flex-col items-center text-white">
                            ...
                            <span className="font-medium">Analyzing stop...</span>
                        </div>
                    </div>
                )}
            </div>
            ...
            {result ? (
                        ...
                        <div className="space-y-2">
                            {result.map((det, idx) => (
                                <div key={idx} className="flex justify-between items-center p-3 bg-white rounded-lg border border-gray-100 shadow-sm">
                                    ...
                                    <span className={`text-sm font-bold ${det.confidence > 0.7 ? 'text-green-600' : 'text-orange-500'}`}>
                ...
                <button
                    ...
                    className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold shadow-lg hover:bg-blue-700 transition transform active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50"
                >
                    <Camera size={24} />
                    Check Stop Status
                </button>
