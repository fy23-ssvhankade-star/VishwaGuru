...
const PlaygroundDetector = ({ onBack }) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.detections.length === 0) {
                alert("No playground hazards detected.");
            }
        } else {
  ...
  return (
    <div className="flex flex-col h-full p-4 max-w-md mx-auto">
      ...
      <h2 className="text-2xl font-bold mb-4 text-green-700">Playground Safety</h2>
      <p className="text-gray-600 mb-4 text-sm">Detect broken equipment or unsafe conditions.</p>
      ...
      <div className="flex justify-center gap-4 mt-auto mb-4">
        {!imgSrc ? (
          ...
          <>
            ...
            >
              {loading ? 'Analyzing...' : 'Detect Hazards'}
            </button>
          </>
        )}
      </div>
