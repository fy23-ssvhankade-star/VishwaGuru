...
const dbPath = process.env.DB_PATH || path.join(__dirname, '../../backend/app.db');
...
export class DailyRefinementJob {
  ...
  public async runRefinement() {
    ...
    try {
      ...
      console.log(`Analyzing ${issuesLast24h.length} issues from the last 24 hours...`);

      // 1. Trend Detection
      const topKeywords = this.trendAnalyzer.getTopKeywords(issuesLast24h, 5);
      const spikes = this.trendAnalyzer.detectCategorySpikes(issuesLast24h, issuesPrev24h);
      const highestSeverityRegion = this.trendAnalyzer.identifyGeographicClustering(issuesLast24h);
      ...
      const newWeights = this.adaptiveWeights.optimizeWeights(issuesLast24h, currentWeights);
      ...
      console.log(`Civic Intelligence Index: ${snapshot.indexScore} (${snapshot.delta >= 0 ? '+' : ''}${snapshot.delta} from yesterday)`);
      if (snapshot.emergingConcerns.length > 0) {
        console.log(`Top Emerging Concern: ${snapshot.emergingConcerns[0].category} (+${snapshot.emergingConcerns[0].increasePercentage}%)`);
      }
      if (snapshot.highestSeverityRegion) {
        console.log(`Highest Severity Region Center: [${snapshot.highestSeverityRegion.latitude}, ${snapshot.highestSeverityRegion.longitude}]`);
      }
    } catch (error) {
  ...
  private getIssues(hoursEnd: number, hoursStart: number = 0): Promise<Issue[]> {
      ...
      this.db.all(query, [], (err, rows) => {
        if (err) {
          ...
          if (err.message.includes("no such table")) {
              console.warn("Table 'issues' does not exist yet. Returning empty issue list.");
              return resolve([]);
          }
          reject(err);
        } else {
...
if (require.main === module) {
  ...
  console.log('Daily Civic Intelligence Refinement Engine scheduled at 00:00 every day.');
