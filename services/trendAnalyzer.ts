...
export class TrendAnalyzer {
  public getTopKeywords(issues: Issue[], limit: number = 5): string[] {
    ...
    const stopWords = new Set(['the', 'is', 'in', 'at', 'of', 'on', 'and', 'a', 'to', 'for', 'with', 'it', 'as', 'by', 'are', 'this', 'that', 'from', 'an', 'be']);

    issues.forEach(issue => {
      if (!issue.description) return;
      const words = issue.description.toLowerCase().match(/\b[a-z]{3,}\b/g) || [];
      words.forEach(word => {
  ...
  public detectCategorySpikes(
    ...
    for (const [category, currentCount] of Object.entries(currentCounts)) {
      ...
      if (prevCount === 0) {
        if (currentCount > 5) { // baseline to avoid 1 issue spike
  ...
  public identifyGeographicClustering(issues: Issue[]): { latitude: number; longitude: number; count: number } | undefined {
    // Basic clustering approach: round lat/lon to ~1.1km (2 decimal places)
    const clusters: Record<string, { count: number, lat: number, lon: number }> = {};
    let maxCluster: { latitude: number; longitude: number; count: number } | undefined;
