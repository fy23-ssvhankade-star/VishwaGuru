...
const URGENCY_PATTERNS = [
  {
    pattern: /\b(now|immediately|urgent|emergency|critical|danger|help)\b/i,
    weight: 20,
  },
...
export class PriorityEngine {
  ...
  private cachedWeights: ModelWeights | null = null;

  constructor(adaptiveWeights?: AdaptiveWeights) {
    this.adaptiveWeights = adaptiveWeights || new AdaptiveWeights();
  }

  private getWeights(): ModelWeights {
    const now = Date.now();
    // 5-second throttle to prevent I/O bottlenecks
    if (!this.cachedWeights || now - this.lastWeightsCheck > 5000) {
      this.cachedWeights = this.adaptiveWeights.loadWeights();
      this.lastWeightsCheck = now;
    }
    return this.cachedWeights;
  }

  public calculateSeverityAndUrgency(
    description: string,
    category: string,
    imageLabels: string[] = [],
  ): AnalysisResult {
    ...
    } = this.calculateSeverity(text);
    let { urgencyScore, reasons: urgencyReasons } = this.calculateUrgency(
      text,
      severityScore,
    );
    const suggestedCategories = this.detectCategories(text);

    const weights = this.getWeights();
    const multiplier = weights.categoryWeights[category];

    if (multiplier && multiplier > 1) {
      severityScore = Math.min(100, Math.round(severityScore * multiplier));
      urgencyScore = Math.min(100, Math.round(urgencyScore * multiplier));
      severityReasons.push(
        `Severity and Urgency scores multiplied by ${multiplier} based on adaptive weights for category: ${category}`,
      );

      if (severityScore >= 90) severity = "Critical";
      else if (severityScore >= 70) severity = "High";
  ...
  private calculateSeverity(text: string) {
    let score = 0;
    let severity: AnalysisResult["severity"] = "Low";
    const reasons: string[] = [];

    const criticalMatches = SEVERITY_KEYWORDS.critical.filter((k) =>
      text.includes(k),
    );
    if (criticalMatches.length > 0) {
      ...
      severity = "Critical";
      reasons.push(
        `Flagged as Critical due to keywords: ${criticalMatches.slice(0, 3).join(", ")}`,
      );
    }

    if (score < 70) {
      const highMatches = SEVERITY_KEYWORDS.high.filter((k) =>
        text.includes(k),
      );
      if (highMatches.length > 0) {
        ...
        reasons.push(
          `Flagged as High Severity due to keywords: ${highMatches.slice(0, 3).join(", ")}`,
        );
      }
    }

    if (score < 40) {
      const mediumMatches = SEVERITY_KEYWORDS.medium.filter((k) =>
        text.includes(k),
      );
      if (mediumMatches.length > 0) {
        ...
        reasons.push(
          `Flagged as Medium Severity due to keywords: ${mediumMatches.slice(0, 3).join(", ")}`,
        );
      }
    }
  ...
  private calculateUrgency(text: string, severityScore: number) {
    ...
    for (const { pattern, weight } of URGENCY_PATTERNS) {
      if (pattern.test(text)) {
        ...
        if (match) {
          reasons.push(
            `Urgency increased by context matching pattern: '${match[0]}'`,
          );
        }
      }
    }
