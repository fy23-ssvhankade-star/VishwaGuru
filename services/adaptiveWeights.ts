...
export class AdaptiveWeights {
  ...
  constructor(configPath: string = path.join(__dirname, '../data/modelWeights.json')) {
  ...
  public optimizeWeights(issues: Issue[], currentWeights: ModelWeights): ModelWeights {
    ...
    issues.forEach(issue => {
      ...
      if (issue.status === 'resolved' || issue.assigned_to) {
        criticalResolutions[issue.category] = (criticalResolutions[issue.category] || 0) + 1;
      }
    });

    for (const [category, count] of Object.entries(criticalResolutions)) {
      if (count > 5) { // Threshold to increase weight
        const increase = 0.5; // Bump weight by 0.5
        const currentWeight = newWeights.categoryWeights[category] || 1;
        newWeights.categoryWeights[category] = Math.min(10, currentWeight + increase); // Cap at 10
      }
    }
    ...
    if (issues.length > 100) {
        newWeights.duplicateThreshold = Math.min(0.95, newWeights.duplicateThreshold + 0.01);
    } else if (issues.length < 20) {
        newWeights.duplicateThreshold = Math.max(0.70, newWeights.duplicateThreshold - 0.01);
    }
  ...
  public saveWeights(weights: ModelWeights): void {
    fs.writeFileSync(this.configPath, JSON.stringify(weights, null, 2), 'utf-8');
  }
}
