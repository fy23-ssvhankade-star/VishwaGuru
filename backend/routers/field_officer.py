...
def get_visit_statistics(db: Session = Depends(get_db)):
    ...
    """
    try:
        # Use SQL aggregates in a single query instead of loading all visits into memory
        stats = db.query(
            ...
            func.sum(case((FieldOfficerVisit.within_geofence == True, 1), else_=0)).label('within_fence'),
            func.sum(case((FieldOfficerVisit.within_geofence == False, 1), else_=0)).label('outside_fence'),
        ...
        verified_visits = int(stats.verified) if stats.verified else 0
        within_geofence_count = int(stats.within_fence) if stats.within_fence else 0
        outside_geofence_count = int(stats.outside_fence) if stats.outside_fence else 0
