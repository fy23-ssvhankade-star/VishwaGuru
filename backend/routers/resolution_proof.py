...
router = APIRouter(prefix="/api/resolution-proof", tags=["Resolution Proof"])
...
def generate_resolution_token(
    request: GenerateRPTRequest, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to generate resolution proof token"
        )
...
def submit_resolution_evidence(
    request: SubmitEvidenceRequest, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to submit resolution evidence"
        )
...
def verify_resolution(grievance_id: int, db: Session = Depends(get_db)):
    """
...
def get_evidence(grievance_id: int, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        logger.error(
            f"Error fetching evidence for grievance {grievance_id}: {e}", exc_info=True
        )
...
async def verify_evidence_audit_blockchain_integrity(
    ...
    audit = await run_in_threadpool(
        ...
        .filter(EvidenceAuditLog.id == audit_id)
        .first()
    )
...
def get_audit_log(grievance_id: int, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        return AuditTrailResponse(
            grievance_id=grievance_id, audit_entries=entries, total_entries=len(entries)
        )
    except Exception as e:
        logger.error(
            f"Error fetching audit log for grievance {grievance_id}: {e}", exc_info=True
        )
...
@router.get(
    "/{evidence_id}/blockchain-verify", response_model=BlockchainVerificationResponse
)
async def verify_resolution_blockchain_integrity(
    ...
    evidence = await run_in_threadpool(
        ...
        .filter(ResolutionEvidence.id == evidence_id)
        .first()
    )
    ...
    token = await run_in_threadpool(
        lambda: db.query(ResolutionProofToken.token_id)
    ...
    chain_content = (
        f"{token_id_str}|{evidence.grievance_id}|{evidence.evidence_hash}|{prev_hash}"
    )
...
def flag_duplicate_evidence(evidence_hash: str, db: Session = Depends(get_db)):
    """
