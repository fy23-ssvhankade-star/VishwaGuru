...
AUDIO_STORAGE_DIR = os.path.join(
    os.path.dirname(__file__), "..", "data", "audio_recordings"
)
...
async def submit_voice_issue(
    audio_file: UploadFile = File(
        ..., description="Audio file with grievance description"
    ),
    ...
    preferred_language: str = Form("auto", description="Preferred language code"),
    manual_description: Optional[str] = Form(
        None, description="Manual correction of description"
    ),
    ...
    try:
        ...
        if prev_hash is None:
            ...
            prev_issue = (
                db.query(Issue.integrity_hash).order_by(Issue.id.desc()).first()
            )
        ...
        logger.info(
            f"Voice issue created: ID={new_issue.id}, Language={voice_result.get('source_language')}, Confidence={voice_result.get('confidence')}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to submit voice issue: {str(e)}"
        )
...
def get_supported_languages():
    ...
    try:
        ...
        return SupportedLanguagesResponse(
            languages=supported_langs, total_count=len(supported_langs)
        )

    except Exception as e:
        logger.error(f"Error getting supported languages: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail="Failed to retrieve supported languages"
        )
...
def get_issue_audio(issue_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if issue.submission_type != "voice" or not issue.audio_file_path:
            raise HTTPException(
                status_code=404, detail="No audio file available for this issue"
            )
        ...
        return FileResponse(
            audio_path, media_type=media_type, filename=os.path.basename(audio_path)
        )
        ...
        logger.error(
            f"Error retrieving audio file for issue {issue_id}: {e}", exc_info=True
        )
