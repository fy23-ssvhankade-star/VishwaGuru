...
class VoiceService:
    ...
    def transcribe_audio(
        self, audio_file: bytes, language: str = "auto"
    ) -> Dict[str, any]:
        """
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Audio conversion failed, trying direct load: {conv_error}"
                )
            ...
            try:
                ...
                if language == "auto":
                    ...
                    transcribed_text = recognizer.recognize_google(
                        audio_data, language=recognition_language, show_all=False
                    )
                    detected_language = language
                    confidence = self._estimate_confidence(transcribed_text)

                logger.info(
                    f"Successfully transcribed audio: language={detected_language}, confidence={confidence}"
                )
    ...
    def translate_text(
        ...
        try:
            ...
            logger.info(
                f"Successfully translated text: {translation.src} -> {translation.dest}"
            )

            return {
                "translated_text": translation.text,
                "source_language": translation.src,
                "source_language_name": SUPPORTED_LANGUAGES.get(
                    translation.src, "Unknown"
                ),
                "target_language": translation.dest,
                "target_language_name": SUPPORTED_LANGUAGES.get(
                    translation.dest, "Unknown"
                ),
                "original_text": text,
                "error": None,
            }
    ...
    def process_voice_grievance(
        self, audio_file: bytes, preferred_language: str = "auto"
    ) -> Dict[str, any]:
        """
        ...
        try:
            ...
            if source_language != "en":
                translation_result = self.translate_text(
                    original_text, source_language=source_language, target_language="en"
                )

                if translation_result["error"]:
                    logger.warning(
                        f"Translation failed, using original text: {translation_result['error']}"
                    )
                else:
