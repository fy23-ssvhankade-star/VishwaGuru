...
AUDIO_CLASS_API_URL = (
    "https://router.huggingface.co/models/MIT/ast-finetuned-audioset-10-10-0.4593"
)

# Speech-to-Text Model (Whisper)
FACIAL_EMOTION_API_URL = (
    "https://router.huggingface.co/models/dima806/facial_emotions_image_detection"
)
...
async def _make_request(client, url, payload):
    try:
        ...
        if response.status_code != 200:
            logger.error(
                f"HF API Error ({url}): {response.status_code} - {response.text}"
            )
            return []
...
async def detect_illegal_parking_clip(
...
async def detect_street_light_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
    labels = [
        "broken streetlight",
...
async def detect_stray_animal_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
...
async def detect_blocked_road_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
    labels = [
        "blocked road",
...
async def detect_tree_hazard_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
    labels = [
        "fallen tree",
...
async def detect_water_leak_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
    labels = [
        "water leak",
...
async def detect_crowd_density_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
    labels = [
        "dense crowd",
...
async def detect_severity_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
    """
    ...
    if isinstance(results, list) and len(results) > 0:
        ...
        level = "Low"
        if label == "critical emergency":
            level = "Critical"
        elif label == "high urgency":
            level = "High"
        elif label == "medium urgency":
            level = "Medium"
...
async def detect_smart_scan_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
    """
...
async def verify_resolution_vqa(
    image: Union[Image.Image, bytes], question: str, client: httpx.AsyncClient = None
):
    """
...
async def detect_depth_map(
    ...
    try:
        ...
        async def do_post(c):
            return await c.post(
                DEPTH_API_URL, headers=headers_bin, content=img_bytes, timeout=30.0
            )
...
async def detect_civic_eye_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
    """
...
async def detect_graffiti_art_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
    """
...
async def detect_traffic_sign_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
    """
...
async def detect_abandoned_vehicle_clip(
    image: Union[Image.Image, bytes], client: httpx.AsyncClient = None
):
    """
...
async def detect_nsfw_content(
    ...
    try:
        ...
        async def do_post(c):
            return await c.post(
                NSFW_API_URL, headers=headers_bin, content=img_bytes, timeout=30.0
            )
