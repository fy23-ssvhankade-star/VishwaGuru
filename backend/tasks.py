...
async def process_action_plan_background(
    issue_id: int, description: str, category: str, language: str, image_path: str
):
    db = SessionLocal()
    try:
        # Generate Action Plan (AI)
        action_plan = await generate_action_plan(
            description, category, language, image_path
        )
...
async def create_grievance_from_issue_background(issue_id: int):
    ...
    try:
        ...
        grievance_data = {
            ...
            "reporter_info": {"email": issue.user_email, "source": issue.source},
        }
        ...
        logger.error(
            f"Error creating grievance from issue {issue_id}: {e}", exc_info=True
        )
    finally:
        db.close()
...
def send_status_notification(
    issue_id: int, old_status: str, new_status: str, notes: str = None
):
    ...
    try:
        ...
        subscriptions = (
            ...
            .filter(
                (PushSubscription.issue_id == issue_id)
                | (PushSubscription.issue_id.is_(None))
            )
            .all()
        )
        ...
        message = status_messages.get(
            new_status, f"Your issue status changed to {new_status}"
        )
