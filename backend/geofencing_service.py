...
def generate_visit_hash(visit_data: dict) -> str:
    ...
    try:
        ...
        if isinstance(check_in_time, datetime):
            ...
            check_in_time_str = check_in_time.replace(microsecond=0).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
        else:
        ...
        visit_hash = hmac.new(
            SECRET_KEY, data_string.encode("utf-8"), hashlib.sha256
        ).hexdigest()
...
def verify_visit_integrity(visit_data: dict, stored_hash: str) -> bool:
    ...
    try:
        ...
        if not is_valid:
            logger.warning(
                f"Visit integrity check FAILED: {computed_hash[:16]}... != {stored_hash[:16]}..."
            )
        else:
...
def calculate_visit_metrics(visits: list) -> dict:
    ...
    try:
        ...
        distances = [
            v.distance_from_site for v in visits if v.distance_from_site is not None
        ]
        ...
        return {
            ...
            "average_distance_from_site": (
                round(avg_distance, 2) if avg_distance else None
            ),
        }
