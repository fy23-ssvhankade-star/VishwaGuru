    ...
    try:
        ...
        for jur_data in jurisdictions_data:
            ...
            existing = (
                ...
                .filter(
                    ...
                    Jurisdiction.responsible_authority
                    == jur_data["responsible_authority"],
                )
                .first()
            )
        ...
        for sla_data in sla_configs_data:
            ...
            if not existing:
                ...
                db.add(sla_config)
                print(
                    f"Created SLA config: {sla_data['severity'].value} - {sla_data['department']} - {sla_data['sla_hours']}h"
                )
...
def test_grievance_creation():
    ...
    for i, grievance_data in enumerate(test_grievances, 1):
        ...
        if grievance:
            print(
                f"✓ Created grievance {i}: {grievance.unique_id} - {grievance.category} - {grievance.assigned_authority}"
            )
        else:
