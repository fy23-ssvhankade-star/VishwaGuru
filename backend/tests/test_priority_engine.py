...
def load_test_data():
    data_path = os.path.join(
        os.path.dirname(__file__), "data", "synthetic_complaints.json"
    )
...
def test_priority_engine_logic(case):
    ...
    # Check Category
    assert (
        case["expected_category"] in result["suggested_categories"]
    ), f"Failed Category for: {description}. Expected {case['expected_category']}, Got {result['suggested_categories']}"
    ...
    ), f"Failed Severity for: {description}. Expected {case['expected_severity']}, Got {result['severity']}"

    # Check Urgency
    if "expected_urgency_min" in case:
        assert (
            result["urgency_score"] >= case["expected_urgency_min"]
        ), f"Failed Urgency Min for: {description}. Expected >= {case['expected_urgency_min']}, Got {result['urgency_score']}"

    if "expected_urgency_max" in case:
        assert (
            result["urgency_score"] <= case["expected_urgency_max"]
        ), f"Failed Urgency Max for: {description}. Expected <= {case['expected_urgency_max']}, Got {result['urgency_score']}"
