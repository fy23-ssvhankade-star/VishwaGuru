...
def populate_db(db: Session, grievance_id: int):
    ...
    for i in range(50):
        db.add(
            GrievanceFollower(grievance_id=grievance_id, user_email=f"user{i}@test.com")
        )
    ...
    for i in range(30):
        db.add(
            ClosureConfirmation(
                ...
                user_email=f"conf_user{i}@test.com",
    ...
    for i in range(10):
        db.add(
            ClosureConfirmation(
                grievance_id=grievance_id,
                user_email=f"disp_user{i}@test.com",
                confirmation_type="disputed",
            )
        )
