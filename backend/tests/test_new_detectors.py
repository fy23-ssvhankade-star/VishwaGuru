...
async def test_detect_traffic_sign_damaged(client):
    ...
    with patch("backend.utils.validate_uploaded_file"), patch(
        "backend.routers.detection._get_cached_result",
        AsyncMock(
            return_value=[
                {"label": "damaged traffic sign", "confidence": 0.95, "box": []}
            ]
        ),
    ):
...
async def test_detect_traffic_sign_clear(client):
    ...
    with patch("backend.utils.validate_uploaded_file"), patch(
        "backend.routers.detection._get_cached_result", AsyncMock(return_value=[])
    ):
        response = client.post(
            "/api/detect-traffic-sign",
            files={"image": ("sign.jpg", img_bytes, "image/jpeg")},
...
async def test_detect_abandoned_vehicle_found(client):
    ...
    with patch("backend.utils.validate_uploaded_file"), patch(
        "backend.routers.detection._get_cached_result",
        AsyncMock(
            return_value=[{"label": "abandoned car", "confidence": 0.92, "box": []}]
        ),
    ):
        response = client.post(
