...
class ActionPlan(BaseModel):
    ...
    relevant_government_rule: Optional[str] = Field(
        None, description="Relevant government policy or rule"
    )
...
class ChatRequest(BaseModel):
    query: str = Field(
        ..., min_length=1, max_length=1000, description="Chat query text"
    )
...
class IssueResponse(IssueSummaryResponse):
    action_plan: Optional[Union[Dict[str, Any], Any]] = Field(
        None, description="Generated action plan"
    )


class IssueCreateRequest(BaseModel):
    description: str = Field(
        ..., min_length=10, max_length=1000, description="Issue description"
    )
...
class IssueStatusUpdateRequest(BaseModel):
    ...
    assigned_to: Optional[str] = Field(
        None, description="Government official/department assigned"
    )
...
class DetectionResponse(BaseModel):
    detections: List[Dict[str, Any]] = Field(
        ..., description="List of detected objects/items"
    )
...
class StatsResponse(BaseModel):
    ...
    pending_issues: int = Field(
        ..., description="Number of open/assigned/in_progress issues"
    )
...
class IssueCreateWithDeduplicationResponse(BaseModel):
    id: Optional[int] = Field(
        None, description="Created issue ID (None if deduplication occurred)"
    )
    ...
    linked_issue_id: Optional[int] = Field(
...
class LeaderboardResponse(BaseModel):
    leaderboard: List[LeaderboardEntry] = Field(
        ..., description="List of top reporters"
    )
...
class EscalationAuditResponse(BaseModel):
    ...
    previous_authority: str = Field(
        ..., description="Previous authority handling the grievance"
    )
...
class EscalationStatsResponse(BaseModel):
    ...
    escalation_rate: float = Field(
        ..., description="Percentage of grievances that were escalated"
    )
...
class FollowGrievanceRequest(BaseModel):
    user_email: str = Field(
        ..., description="Email of the user following the grievance"
    )
...
class RequestClosureResponse(BaseModel):
    ...
    confirmation_deadline: datetime = Field(
        ..., description="Deadline for community confirmation"
    )
    total_followers: int = Field(
        ..., description="Number of followers who will be notified"
    )
...
class ConfirmClosureRequest(BaseModel):
    ...
    confirmation_type: str = Field(
        ...
        description="Type: 'confirmed' or 'disputed'",
    )
    reason: Optional[str] = Field(
        None, max_length=500, description="Reason for dispute (optional)"
    )
...
class ConfirmClosureResponse(BaseModel):
    ...
    current_confirmations: int = Field(
...
class ClosureStatusResponse(BaseModel):
    grievance_id: int = Field(..., description="Grievance ID")
    pending_closure: bool = Field(
        ..., description="Whether closure is pending confirmation"
    )
    closure_approved: bool = Field(..., description="Whether closure has been approved")
    total_followers: int = Field(..., description="Total number of followers")
    ...
    confirmation_deadline: Optional[datetime] = Field(
        None, description="Deadline for confirmations"
    )
...
class BlockchainVerificationResponse(BaseModel):
    ...
    current_hash: Optional[str] = Field(
        None, description="Current integrity hash stored in DB"
    )
    computed_hash: str = Field(
        ...,
        description="Hash computed from current issue data and previous issue's hash",
    )
...
class GenerateRPTRequest(BaseModel):
    ...
    authority_email: str = Field(
        ..., description="Email of the authority resolving the grievance"
    )
    geofence_radius_meters: Optional[float] = Field(
        200.0, ge=50, le=1000, description="Geofence radius in meters"
    )
...
class RPTResponse(BaseModel):
    ...
    token_signature: str = Field(
        ..., description="Cryptographic signature of the token"
    )
    message: str = Field(..., description="Status message")
...
class SubmitEvidenceRequest(BaseModel):
    ...
    evidence_hash: str = Field(
        ...
        min_length=64,
        max_length=64,
        description="SHA-256 hash of the evidence media file",
    )
    gps_latitude: float = Field(
        ..., ge=-90, le=90, description="GPS latitude of capture location"
    )
    gps_longitude: float = Field(
        ..., ge=-180, le=180, description="GPS longitude of capture location"
    )
    ...
    device_fingerprint_hash: Optional[str] = Field(
        None, description="Hash of device fingerprint"
    )
...
class EvidenceResponse(BaseModel):
    ...
    verification_status: str = Field(
        ...,
        description="Verification status: pending, verified, flagged, fraud_detected",
    )
...
class VerificationResponse(BaseModel):
    ...
    is_verified: bool = Field(
        ..., description="Whether the resolution is cryptographically verified"
    )
    verification_status: str = Field(
        ..., description="Status: pending, verified, flagged, fraud_detected"
    )
    resolution_timestamp: Optional[datetime] = Field(
        None, description="When the grievance was resolved"
    )
    location_match: bool = Field(
        ..., description="Whether evidence GPS matches grievance geofence"
    )
    evidence_integrity: bool = Field(..., description="Whether evidence hash is intact")
    evidence_hash: Optional[str] = Field(
        None, description="SHA-256 hash fingerprint for transparency"
    )
...
class EvidenceAuditLogResponse(BaseModel):
    ...
    action: str = Field(
        ..., description="Action: created, verified, flagged, fraud_detected"
    )
    details: Optional[str] = Field(None, description="Additional details")
    actor_email: Optional[str] = Field(
        None, description="Actor who triggered the action"
    )
...
class AuditTrailResponse(BaseModel):
    grievance_id: int = Field(..., description="Grievance ID")
    audit_entries: List[EvidenceAuditLogResponse] = Field(
...
class DuplicateCheckResponse(BaseModel):
    is_duplicate: bool = Field(
        ..., description="Whether the evidence hash is a duplicate"
    )
    duplicate_grievance_ids: List[int] = Field(
        default_factory=list, description="Grievance IDs with matching hash"
    )
...
class VoiceTranscriptionResponse(BaseModel):
    ...
    original_text: Optional[str] = Field(
        None, description="Transcribed text in original language"
    )
    translated_text: Optional[str] = Field(
        None, description="Translated text (English)"
    )
    source_language: Optional[str] = Field(None, description="Detected language code")
    source_language_name: Optional[str] = Field(
        None, description="Detected language name"
    )
    confidence: float = Field(..., description="Transcription confidence score (0-1)")
    manual_correction_needed: bool = Field(
        ..., description="Flag indicating if manual correction is needed"
    )
...
class TextTranslationRequest(BaseModel):
    ...
    source_language: str = Field(
        "auto", description="Source language code ('auto' for detection)"
    )
...
class TextTranslationResponse(BaseModel):
    ...
    source_language_name: Optional[str] = Field(
        None, description="Source language name"
    )
    target_language: Optional[str] = Field(None, description="Target language")
    target_language_name: Optional[str] = Field(
        None, description="Target language name"
    )
    original_text: str = Field(..., description="Original text")
    error: Optional[str] = Field(
        None, description="Error message if translation failed"
    )
...
class VoiceIssueCreateRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="Issue description (for manual corrections)"
    )
    category: IssueCategory = Field(..., description="Issue category")
    user_email: Optional[str] = Field(None, description="User's email address")
    latitude: Optional[float] = Field(
    ...
    submission_type: str = Field(
        "text", pattern="^(text|voice)$", description="Submission type"
    )
    original_language: Optional[str] = Field(None, description="Original language code")
    original_text: Optional[str] = Field(
        None, description="Original text in regional language"
    )
...
class SupportedLanguagesResponse(BaseModel):
    ...
    languages: Dict[str, str] = Field(
        ..., description="Dictionary of language code to language name"
    )
...
class OfficerCheckInRequest(BaseModel):
    ...
    grievance_id: Optional[int] = Field(
        None, description="Optional grievance ID if linked"
    )
    officer_email: str = Field(..., description="Officer's email address")
    officer_name: str = Field(
        ..., min_length=2, max_length=100, description="Officer's full name"
    )
    officer_department: Optional[str] = Field(
        None, max_length=100, description="Department name"
    )
    officer_designation: Optional[str] = Field(
        None, max_length=100, description="Officer's designation"
    )
    check_in_latitude: float = Field(
        ..., ge=-90, le=90, description="Check-in GPS latitude"
    )
    check_in_longitude: float = Field(
        ..., ge=-180, le=180, description="Check-in GPS longitude"
    )
    visit_notes: Optional[str] = Field(
        None, max_length=1000, description="Visit notes/observations"
    )
    geofence_radius_meters: Optional[float] = Field(
        100.0, ge=10, le=1000, description="Acceptable distance from site (meters)"
    )
...
class OfficerCheckOutRequest(BaseModel):
    ...
    check_out_latitude: float = Field(
        ..., ge=-90, le=90, description="Check-out GPS latitude"
    )
    check_out_longitude: float = Field(
        ..., ge=-180, le=180, description="Check-out GPS longitude"
    )
    visit_duration_minutes: Optional[int] = Field(
        None, ge=0, le=1440, description="Visit duration in minutes"
    )
    additional_notes: Optional[str] = Field(
        None, max_length=1000, description="Additional notes at check-out"
    )
...
class FieldOfficerVisitResponse(BaseModel):
    ...
    distance_from_site: Optional[float] = Field(
        None, description="Distance from site in meters"
    )
    within_geofence: bool = Field(
        ..., description="Whether check-in was within geofence"
    )
...
class VisitHistoryResponse(BaseModel):
    ...
    visits: List[PublicFieldOfficerVisitResponse] = Field(
        ..., description="List of visits (PII removed for public access)"
    )
...
class VisitStatsResponse(BaseModel):
    ...
    average_distance_from_site: Optional[float] = Field(
        None, description="Average distance in meters"
    )
